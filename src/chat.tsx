...
function ErrorMessage({ error }: { error: string }) {
    ...
    if (code === "4006") {
        ...
        description = "You've used your daily free allocation of 10,000 neurons. Please upgrade to Cloudflare's Workers Paid plan to continue.";
    }
        ...
        <div className="flex justify-start">
            <Surface className="max-w-[85%] px-4 py-3 rounded-xl rounded-bl-md bg-kumo-danger/10 border border-kumo-danger/30">
                <div className="flex items-start gap-3">
                    <XCircleIcon size={18} className="text-kumo-danger flex-shrink-0 mt-0.5" />
                    <div>
                        <div className="font-bold text-kumo-danger">
                            {title}
                        </div>
                        <div className="text-sm text-kumo-default mt-1">
                            {description}
                        </div>
                    </div>
                </div>
            ...
            <Surface className="max-w-[85%] px-4 py-3 rounded-2xl rounded-bl-md bg-kumo-base">
                <div className="flex items-center gap-2">
                    <div className="flex gap-1">
                        <div className="w-2 h-2 bg-kumo-default rounded-full animate-bounce" style={{ animationDelay: '0ms' }} />
                        <div className="w-2 h-2 bg-kumo-default rounded-full animate-bounce" style={{ animationDelay: '150ms' }} />
                        <div className="w-2 h-2 bg-kumo-default rounded-full animate-bounce" style={{ animationDelay: '300ms' }} />
                    </div>
...
export const Chat = forwardRef<any, ChatProps>((_, ref) => {
                            ...
                            <div key={message.id} className="space-y-2">
                                    ...
                                    <ErrorMessage
                                        error={(
                                            textParts
                                                ...
                                                .find((text) => text && text.includes("error")) || "An error occurred"
                                        ).trim()}
                                    />
                                )}
                                ...
                                {message.parts
                                    ...
                                    .map((part, i) => {
                                        ...
                                        if (!isUser && text.match(/^\d+:/) && text.includes("error")) {
                                            return null;
                                        }

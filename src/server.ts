...
Never invent product details, pricing, or policy. Never promise specific outcomes.`,
      ...
      tools: {
        ...
        appendTranscript: tool({
          ...
          execute: async ({ id, additionalTranscript }) =>
            TicketModule.appendTranscript(this.env, id, additionalTranscript)
        }),
        ...
        findRecentTickets: tool({
          ...
          inputSchema: z.object({
            ...
            limit: z
              .number()
              .optional()
              .describe("Maximum number of tickets, defaults to 10")
          }),
          execute: async ({ customer_email, limit }) =>
            TicketModule.findRecentTickets(this.env, customer_email, limit)
        }),
        ...
        findCustomerHistory: tool({
          ...
          execute: async ({ customer_email }) =>
            TicketModule.findCustomerHistory(this.env, customer_email)
        }),
        ...
        identifyCustomer: tool({
          ...
          execute: async ({ customer_email }) =>
            TicketModule.identifyCustomer(this.env, customer_email)
        }),
        ...
        pass it as pageUrl to skip the index lookup and fetch the page directly.`,
          inputSchema: z.object({
            productKey: z
              .string()
              .optional()
              .describe("Product key for technical questions"),
            directKey: z
              .string()
              .optional()
              .describe("Direct key for legal, pricing, abuse questions"),
            pageUrl: z
              .string()
              .optional()
              .describe("Specific page URL if known")
          }),
          execute: async (params) => {
            ...
            if (params.pageUrl) {
              ...
              if (!url)
                throw new Error(`Invalid productKey: ${params.productKey}`);
            } else if (params.directKey) {
              url =
                NON_PRODUCT_DOCS[
                  params.directKey as keyof typeof NON_PRODUCT_DOCS
                ];
              ttl;
              if (!url)
                throw new Error(`Invalid directKey: ${params.directKey}`);
            } else {
  ...
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (url.pathname === "/api/tickets") {
      if (request.method === "GET") {
        ...
        let tickets;
        if (customerEmail) {
          tickets = await TicketModule.findRecentTickets(
            env,
            customerEmail,
            50
          );
          ...
          const rows = await db
            .prepare("SELECT * FROM tickets ORDER BY created_at DESC LIMIT 100")
            .all();
